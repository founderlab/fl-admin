...
var _componentsInputsBelongs_to = require('../../components/inputs/belongs_to');

var _componentsInputsBelongs_to2 = _interopRequireDefault(_componentsInputsBelongs_to);

var _componentsInputsHas_many = require('../../components/inputs/has_many');

var _componentsInputsHas_many2 = _interopRequireDefault(_componentsInputsHas_many);

function createRelatedField(relation_field, model) {
    ...
    _createClass(RelatedField, [{
      ...
      value: function render() {
        ...
        if (relation_field.type === 'belongsTo' /*|| relation_field.type === 'hasOne' */) {
            return _react2['default'].createElement(_componentsInputsBelongs_to2['default'], { relation_field: relation_field, model: model, model_store: model_store, input_props: input_props });
          }

        if (relation_field.type === 'hasMany' || relation_field.type === 'hasOne') {
          return _react2['default'].createElement(_componentsInputsHas_many2['default'], { relation_field: relation_field, model: model, model_store: model_store, input_props: input_props });
        }

        (0, _warning2['default'])(false, '[fl-admin] Relation does not have a known type: ' + relation_field.type);
        return null;
      }
    }], [{
      key: 'fetchData',
      value: function fetchData(store, callback) {
        var query = _defineProperty({}, related_field.foreign_key, model.id);
