...
function createModelEditor(model_admin) {
  ...
  return (function (_Component) {
    ...
    ModelEditor.prototype.render = function render() {
      ...
      _lodash2['default'].forEach(visible_items, function (model) {
        _lodash2['default'].forEach(model_admin.fields, function (f, key) {
          if (f.type.toLowerCase() === 'datetime' && model[key]) {
            model[key] = _moment2['default'](model[key]).format('L LT');
          } else if (f.type.toLowerCase() === 'date' && model[key]) {
            model[key] = _moment2['default'](model[key]).format('L');
          }
        });
      });
