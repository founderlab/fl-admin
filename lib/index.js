...
var _containersGeneratorsPagination = require('./containers/generators/pagination');

var _containersGeneratorsPagination2 = _interopRequireDefault(_containersGeneratorsPagination);
...
function createModelAdmin(options, model_descriptor) {
  ...
  _lodash2['default'].forEach(relation_fields, function (relation, key) {
    ...
    _lodash2['default'].defaults(admin_field, _lodash2['default'].pick(relation, 'type', 'virtual_id_accessor', 'components'), { components: {} });
  ...
  if (!model_admin.components.Pagination) model_admin.components.Pagination = (0, _containersGeneratorsPagination2['default'])(model_admin);
...
function configure(_options) {
  ...
  _lodash2['default'].forEach(model_admins, function (model_admin) {
    _lodash2['default'].forEach(model_admin.relation_fields, function (admin_field) {
      ...
      (0, _warning2['default'])(admin_field.model_admin, '[fl-admin] configure: Couldnt find model_admin for the relation ' + admin_field.key + ' of ' + model_admin.name);
      if (!admin_field.RelatedField) admin_field.RelatedField = (0, _containersGeneratorsRelated_field2['default'])(admin_field);
    });
  });
