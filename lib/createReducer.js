...
var _flReduxUtils = require('fl-redux-utils');

function createReducer(modelAdmin) {

  var pagination = _flReduxUtils.createPaginationReducer(modelAdmin.actionType);
  ...
  return function reducer() {
    ...
    switch (action.type) {
      ...
      case modelAdmin.actionType + '_SAVE_SUCCESS':
        console.log('_SAVE_SUCCESS', action);
        console.log(state.merge({
          loading: false,
          errors: {},
          lastSaved: action.model,
          models: (_models = {}, _models[action.model.id] = action.model, _models)
        }).toJSON());
        return state.merge({
          loading: false,
          errors: {},
          lastSaved: action.model,
          models: (_models2 = {}, _models2[action.model.id] = action.model, _models2)
        });
